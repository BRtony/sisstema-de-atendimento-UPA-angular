...
//   {id: 20, name: 'Calcium'},
// ];
function listarTodos(): Upa[] {
  const atendimentos = localStorage['atendimentos'];
  return atendimentos ? JSON.parse(atendimentos) : [];
}

/**
 * Data source for the ListarAtendimentos view. This class should
 * encapsulate all logic for fetching and manipulating the displayed data
 * (including sorting, pagination, and filtering).
 */
export class ListarAtendimentosDataSource extends DataSource<Upa> {
  data: Upa[] = listarTodos();
   ...
   */
  private getPagedData(data: Upa[]): Upa[] {
    if (this.paginator) {
   ...
   * this would be replaced by requesting the appropriate data from the server.
   */
  private getSortedData(data: Upa[]): Upa[] {
